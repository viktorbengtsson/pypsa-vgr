...
def create_and_store_data_analytics(config):
    ...
    if os.path.isfile(f"../{DATA_PATH}/network.pkl"):
        print("Analytics: Files already exists, continue")
        return
    ...
    data_collection = {
        ...
        "capacity_monthly": _capacity_monthly(NETWORK, parameters),
        "table": _table(NETWORK, parameters),
        "energy_compare": _energy_compare(NETWORK, parameters),
        "widgets": _widgets(NETWORK, STATISTICS, parameters),
    }
